...
if(isset($_SESSION['Email']))
{
    ...
    $stmt = "INSERT INTO snakegame (Email, score) VALUES ('$db_Email', '$Score')";
    ...
    //showing progress
        $progress="SELECT AVG(score) AS avg_score
               ...
               WHERE Email = '$db_Email' AND id NOT IN (
                SELECT MAX(id) FROM snakegame WHERE Email = '$db_Email'
    )"; 
    ...
    if($result1)
    {
        echo "Data inserted into snakegame table for user " . $db_Email;
    }
    else
    {
        echo "Error inserting data into snakegame table: " . mysqli_error($con);
    }
}
else
{
